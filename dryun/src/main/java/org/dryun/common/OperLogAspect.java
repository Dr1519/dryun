...
import org.dryun.untils.IPUtils;
...
public class OperLogAspect {
     ...
     */
    @Pointcut("@annotation(org.dryun.common.OperLog)")
     ...
     */
    @Pointcut("execution(* org.dryun.domain.api.VueApi..*.*(..))")
    ...
    public void saveOperLog(JoinPoint joinPoint, Object keys) {
        ...
        try {
            operlog.setOperId(UUIDUtils.getUUID()); // 主键ID
...
//            operlog.setOperUserName(UserShiroUtil.getCurrentUserName()); // 请求用户名称
            operlog.setOperUserId("100001");
            operlog.setOperUserName("晨初");
...
//            operlog.setOperVer(operVer); // 操作版本
            operationService.addOperLog(operlog);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void saveExceptionLog(JoinPoint joinPoint, Throwable e) {
        ...
        try {
            ...
            // 获取切入点所在的方法
            Method method = signature.getMethod();
            excepLog.setExcId(UUIDUtils.getUUID());
            // 获取请求的类名
            String className = joinPoint.getTarget().getClass().getName();
            // 获取请求的方法名
            String methodName = method.getName();
...
//            excepLog.setExcUserId(UserShiroUtil.getCurrentUserLoginName()); // 操作员ID
//            excepLog.setExcUserName(UserShiroUtil.getCurrentUserName()); // 操作员名称
            excepLog.setExcUrl(request.getRequestURI()); // 操作URI
            excepLog.setExcIp(IPUtils.getIpAddr(request)); // 操作IP
//            excepLog.setExcVer(operVer); // 操作版本号
            excepLog.setExcCreateTime(new Date()); // 发生异常时间

            exceptionService.addExcLog(excepLog);
